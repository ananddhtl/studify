...
<div class="content d-flex flex-column flex-column-fluid" id="kt_content" >
                      ...
                      <div style="overflow: auto hidden; min-width: 943px !important;">
                        ...
                        <div class="table-responsive" style="height: 220px;
                           ...
                           <table id="kt_applied_application_table" class="table table-row-bordered table-row-dashed gy-4 align-middle fw-bolder">
                                ...
                                <tr>
                                    @php 
                                    $university = App\Models\addInstitution::where(['id' => $appliedCourses->insitution_id])->first();
                                    @endphp
                                    @if($university)
                                    <td><a href="https://studify.au/public/InstitutionImage/167576484322.jpg" class="fs-6 text-gray-800 text-hover-primary "><img src="https://studify.au/public/InstitutionImage/167576484322.jpg" style="width: 100px;"><br>{{$university->universirty_name}}</a></td>
                                    @else
                                    <td>--</td>
                                    @endif
                                    @php 
                                    $course = App\Models\addCoursesModel::where(['id' => $appliedCourses->course_id])->first();
                                    @endphp
                                    @if($course)
                                    <td>{{$course->c_name}}</td>
                                    @else
                                    ...
                                    @php 
                                   $fees = App\Models\addFees::where(['course_id' => $appliedCourses->course_id])->where(['institution_detail_id' => $appliedCourses->insitution_id])->where(['type' => $course->type])->first();
                                   @endphp
                                 @if($fees)
                                    ...
                                    <td style="color:green;"><div class="dropdown">
    ...
    <ul class="dropdown-menu"style="top: 40px !important;" id="mee">
      ...
      <li><a href="{{url('/universitiesdetails/'.$appliedCourses->insitution_id)}}">View</a></li>
      <li><a href="{{url('/student/university-course-delete/'.$appliedCourses->id)}}">Delete</a></li>
    </ul>
  </div></td>
                                    @endif
