...
<form id="First" method="POST" action="{{url('institution/add-members')}}" accept-charset="UTF-8" class="card-body py-20 w-100 w-xl-700px x-9 fv-plugins-bootstrap5 fv-plugins-framework" id="application_manager_form" novalidate="novalidate" enctype="multipart/form-data">
...
<div class="use">
<div class="list">
    @if($groupmembers->type == 'Agent')
    @php
   $data = App\Models\AgentModel::where(['id' => $groupmembers->member_id])->first();
@endphp 
@if($data)
...
<div class="text">
  ...
  @if($data)
    ...
    <br>
    <br>
    <span>{!!$groupmembers->message!!}</span>
    <br>
    @php
    \Carbon\Carbon::setLocale('ru');
      @endphp
 <span>{{ \Carbon\Carbon::parse($groupmembers->created_at)->format('d F Y') }}</span>
  </p>
  </div>
  @elseif($groupmembers->type == 'Insitution')
  @php
   $data = App\Models\InstitutionModel::where(['id' => $groupmembers->member_id])->first();
...
<div class="text">
 ...
 <span>{{ \Carbon\Carbon::parse($groupmembers->created_at)->format('d F Y') }}</span>
  ...
  @else
  @php
   $data = App\Models\StudentModel::where(['id' => $groupmembers->member_id])->first();
@endphp 
@if($data)
