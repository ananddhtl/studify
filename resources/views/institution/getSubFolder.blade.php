  ...
  <button type="submit"><i class="fa fa-search"></i></button>
    ...
    <div class="col-sm-8">
     @foreach($getfile as $getfiles)
      ...
      @php
                                        $key = 'institution/institution_'.$institution_id.'/'.$getfiles->filename;
 ...
 <th scope="col">File Size</th>
  ...
  <tbody>
    ...
    <tr>
      ...
      @endif      </th>
      @if($folders->foldername != '')
      <td><a href="{{url('/institution/get-files/'.$folders->id)}}">{{$folders->foldername}}</a></td>
      @else
      ...
      @php
                                        $key = 'institution/institution_'.$institution_id.'/'.$folders->filename;
                                        $client = Storage::disk('s3')->getDriver()->getAdapter()->getClient();
                                        $bucket = 'studifynewbucket';
                                        ...
                                        $request = $client->createPresignedRequest($command, '+20 minutes');
                                        $presignedUrl = (string)$request->getUri();
                                        $sizee = Storage::disk('s3')->size('institution/institution_'.$institution_id.'/'.$folders->filename);
...
<!--end::Page-->
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content" id="width5">
        ...
        <form method="POST" action="{{url('/institution/create-document-sub-folder')}}" accept-charset="UTF-8" class="card-body w-100 w-xl-700px x-9 fv-plugins-bootstrap5 fv-plugins-framework" id="application_manager_form" novalidate="novalidate" enctype="multipart/form-data">
            @csrf
                               <div class="w-100">
                                <!--begin::Heading-->
                            ...
                            <div class="row fv-row mb-12">
                                <div class="row">
                                    <input type="hidden" id="id" name="institution_id" value="{{$institution_id}}" >
        ...
        <div class="modal fade show in"  data-backdrop="false" id="exampleModal3" role="dialog">
    <div class="modal-dialog">
...
$(document).ready(function(){
 $(document).on('change', '#file', function(){
  ...
  form_data.append("folder_id", folder_id);
   $.ajax({
    url: '/institution/create-sub-folder-file',
    method:"POST",
    data: form_data,
    contentType: false,
    cache: false,
