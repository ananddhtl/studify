...
<div class="content d-flex flex-column flex-column-fluid" id="kt_content">
    ...
    <div class="col-sm-8">
     @foreach($getfile as $getfiles)
      ...
      @php
                                        $key = 'agent/agent_'.$agent_id.'/'.$getfiles->filename;
    ...
    <div class="col-sm-4">
    @php $ext = pathinfo($getfiles->filename, PATHINFO_EXTENSION); @endphp
    @if($ext == 'pdf')
...
@endforeach
  ...
  <form method="POST" action="{{url('/agent/create-folder-file')}}" accept-charset="UTF-8" class="hhhh" id="application_manager_form" novalidate="novalidate" enctype="multipart/form-data">
            @csrf
        <div class="doc1">
 ...
 <th scope="col">File Size</th>
  ...
  <tbody>
    ...
    <tr>
      ...
      @endif      </th>
      @if($folders->foldername != '')
      <td><a href="{{url('/agent/get-files/'.$folders->id)}}">{{$folders->foldername}}</a></td>
      @else
      ...
      @php
                                        $key = 'agent/agent_'.$agent_id.'/'.$folders->filename;
                                        $client = Storage::disk('s3')->getDriver()->getAdapter()->getClient();
                                        $bucket = 'studifynewbucket';
                                        ...
                                        $request = $client->createPresignedRequest($command, '+20 minutes');
                                        $presignedUrl = (string)$request->getUri();
                                        if ( Storage::disk('s3')->exists('agent/agent_'.$id.'/'.$folders->filename))
                                     {
                                        $sizee = Storage::disk('s3')->size('/agent/agent_'.$id.'/'.$folders->filename);
...
<!--end::Page-->
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content" id="width5">
        ...
        <form method="POST" action="{{url('/agent/create-document-sub-folder')}}" accept-charset="UTF-8" class="card-body w-100 w-xl-700px x-9 fv-plugins-bootstrap5 fv-plugins-framework" id="application_manager_form" novalidate="novalidate" enctype="multipart/form-data">
            @csrf
                            ...
                            <div class="row fv-row mb-12">
                                <div class="row">
                                    <input type="hidden" id="id" name="agent_id" value="{{$agent_id}}" >
        ...
        <div class="modal fade show in"   style="display:none;" data-backdrop="false" id="exampleModal3" role="dialog">
    <div class="modal-dialog">
  ...
  <li class="list"><a class="deletefile" href="#"><i class="fa fa-trash" aria-hidden="true" id="icn"></i>Delete </a></li>
                    ...
                    <div class="modal fade"  style="display:none;" id="exampleModalCenter1" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
...
" >
  ...
  <div class="modal-dialog " role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
      <form method="POST" action="{{url('/agent/update-document-folder')}}" accept-charset="UTF-8" class="card-body w-100 w-xl-700px x-9 fv-plugins-bootstrap5 fv-plugins-framework" id="application_manager_form" novalidate="novalidate" enctype="multipart/form-data">
            @csrf
